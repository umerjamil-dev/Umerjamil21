...
const Topbar = ({ onMenuClick }) => {
  ...
  return (
    <div className="h-16 md:h-24 bg-white/80 backdrop-blur-2xl px-4 md:px-10 flex items-center justify-between sticky top-0 z-30 border-b border-black/[0.04]">
      ...
      <div className="flex items-center gap-3 w-full md:w-auto">
        ...
        <div className="relative w-full md:w-[480px] group flex-1 md:flex-none">
          ...
          <input
            type="text"
            placeholder="Search..."
            className="w-full pl-9 md:pl-12 pr-10 md:pr-16 py-2.5 md:py-4 bg-black/[0.02] hover:bg-black/[0.04] focus:bg-white focus:ring-[4px] focus:ring-[var(--desert-gold)]/20 border border-transparent focus:border-[var(--desert-gold)]/50 rounded-xl text-[13px] md:text-sm transition-all duration-300 outline-none placeholder-gray-400 font-medium text-gray-800 shadow-sm focus:shadow-md"
          />
          <div className="absolute inset-y-0 right-3 md:right-4 flex items-center pointer-events-none">
            <div className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg md:rounded-xl bg-black/5 text-[9px] md:text-[10px] font-bold text-gray-400 uppercase tracking-widest hidden sm:flex border border-black/5">
              <Command size={12} /> K
            </div>
          </div>
        </div>
      ...
      <div className="flex items-center gap-3 md:gap-8 ml-3 shrink-0">
        ...
        <div className="flex items-center gap-1 md:gap-2">
          <button className="relative w-9 h-9 md:w-12 md:h-12 flex items-center justify-center text-gray-400 hover:text-[var(--desert-gold)] hover:bg-[var(--desert-gold)]/5 rounded-xl transition-all duration-300 border border-transparent hover:border-[var(--desert-gold)]/20">
            <Bell size={18} strokeWidth={1.5} className="md:w-[22px] md:h-[22px]" />
            <span className="absolute top-2 md:top-3 right-2 md:right-3 w-2 h-2 md:w-2.5 md:h-2.5 bg-[var(--desert-gold)] shadow-[0_0_10px_var(--desert-gold)] border-2 border-white rounded-full animate-pulse"></span>
          </button>
        ...
        <div className="hidden sm:block w-px h-8 md:h-10 bg-black/[0.05]"></div>

        {/* User Profile - VIP Style */}
        <div className="flex items-center gap-2 md:gap-4 cursor-pointer group hover:bg-black/[0.02] p-1.5 md:p-2 md:pr-5 rounded-[1rem] md:rounded-[1.25rem] transition-all border border-transparent hover:border-[var(--desert-gold)]/20">
          <div className="text-right hidden sm:block">
            <p className="text-[12px] md:text-[13px] font-manrope font-extrabold text-gray-800 leading-none group-hover:text-[var(--desert-gold)] transition-colors">
          ...
          <div className="relative">
            <div className="w-8 h-8 md:w-11 md:h-11 rounded-full md:rounded-[1.25rem] bg-gradient-to-br from-[#1E293B] to-[#0F172A] flex items-center justify-center text-white shadow-lg shadow-black/10 group-hover:scale-105 group-hover:shadow-[0_4px_15px_rgba(212,175,55,0.4)] transition-all border border-black/10">
              <User size={16} strokeWidth={2} className="md:w-[20px] md:h-[20px] text-[var(--desert-gold)] transition-colors" />
            </div>

            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white border-b border-slate-100">
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/4">Hospitality Node</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/5">Occupant</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/5">Temporal Window</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/5">Inventory Configuration</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] text-right">Protocol Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  <tr><td colSpan="5" className="py-20 text-center text-sm font-medium uppercase tracking-widest text-slate-400 opacity-50">Querying Hospitality Ledger...</td></tr>
                ) : paginatedData.length === 0 ? (
                  <tr><td colSpan="5" className="py-20 text-center text-sm font-medium uppercase tracking-widest text-slate-400 opacity-50">No hospitality records found.</td></tr>
                  ...
                  <tr key={item.id} className="group hover:bg-slate-50 transition-all cursor-pointer">
                    <td className="px-8 py-6 text-black">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-base font-manrope font-medium text-slate-900 tracking-tight leading-none mb-1">{item.hotel_name}</p>
                          <p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest flex items-center gap-1.5 mt-1.5">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1.5">
                        <div className="flex items-center gap-2 text-[10px] font-medium text-slate-600 uppercase tracking-widest bg-emerald-50 text-emerald-700 px-2 py-1 rounded inline-flex w-fit">
                          <Navigation size={10} className="transform rotate-90" />
                          IN: {item.check_in}
                        </div>
                        <div className="flex items-center gap-2 text-[10px] font-medium text-slate-500 uppercase tracking-widest bg-slate-100 text-slate-600 px-2 py-1 rounded inline-flex w-fit">
                          <Navigation size={10} className="transform -rotate-90" />
                          OUT: {item.check_out}
                        </div>
                      </div>
                    ...
                    <td className="px-8 py-6 text-black">
                      <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-xl text-[10px] font-medium text-slate-600">
                        <BedDouble size={14} /> {item.rooms}
                      </div>
                    </td>
                    <td className="px-8 py-6 text-right">
                      <div className="flex items-center justify-end gap-4 text-black">
                        <span className={`px-4 py-1.5 rounded-full text-[8.5px] font-medium uppercase tracking-[0.2em] inline-flex items-center gap-2 ${item.status === 'Confirmed' ? 'bg-[var(--sacred-emerald)]/10 text-[var(--sacred-emerald)]' :

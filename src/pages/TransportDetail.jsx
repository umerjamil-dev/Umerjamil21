          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                <span className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em]">Transit Duty</span>
                <div className="w-1.5 h-1.5 rounded-full bg-slate-900"></div>
                <span className="text-[10px] font-medium text-slate-900 uppercase tracking-widest">UNIT: {transport.id}</span>
              </div>
              <h1 className="text-2xl font-manrope font-medium text-slate-900 tracking-tight">Vehicle Protocol: {transport.vehicle || 'Unassigned'}</h1>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl p-10 shadow-sm border border-slate-100 relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between mb-12 relative z-10">
                ...
                <span className="px-4 py-1.5 bg-slate-900 text-white rounded-full text-[10px] font-medium uppercase tracking-widest border border-slate-800 uppercase italic">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
                <div className="space-y-8">
                  <div>
                    ...
                    <p className="text-xl font-mono font-medium text-slate-900 tracking-tighter bg-black text-white px-5 py-2.5 rounded-xl inline-block uppercase italic">
                      {transport.plate_number || 'TBA'}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-8">
                    <div>
                      <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-2">Service Date</p>
                      <p className="text-sm font-medium text-slate-900">{transport.date_of_issue || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-2">Registry Time</p>
                      <p className="text-sm font-medium text-slate-900">{transport.time || 'N/A'}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div>
                    ...
                    <p className="text-lg font-manrope font-medium text-slate-900">{transport.driver || 'Verification Pending'}</p>
                    <div className="flex items-center gap-2 text-slate-500 mt-2">
                      <Phone size={12} />
                      <p className="text-[11px] font-medium tracking-widest uppercase italic">{transport.driver_phone || 'No Contact'}</p>
                    </div>
                  </div>
                  <div>
                    <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-3">Principal Passenger</p>
                    <p className="text-sm font-medium text-slate-900 italic underline decoration-slate-200 decoration-2 underline-offset-4">{customerName}</p>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-xs font-medium text-slate-900 mb-2 uppercase tracking-tight">{transport.package_title || 'Custom Arrangement'}</p>
                  <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest">Financial Index: PKR {parseFloat(transport.total_amount || 0).toLocaleString()}</p>
                </div>
                <Link to={`/bookings/${transport.booking_id}`} className="text-[10px] font-medium text-slate-400 uppercase tracking-widest hover:text-black transition-colors mt-4 italic underline underline-offset-2">
                  View Master Record →
                </Link>
              </div>
              <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm">
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-slate-50 rounded-xl border border-slate-100">
                    <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1">Issue Metadata</p>
                    <p className="text-xs font-medium text-slate-700">{transport.date_of_issue || 'Pending Validation'}</p>
                  </div>
                </div>
              </div>
            </div>
          ...
          <div className="space-y-8">
            <div className="bg-slate-900 rounded-2xl p-8 text-white shadow-xl relative overflow-hidden group">
              ...
              <p className="text-[10px] font-medium text-white/30 uppercase tracking-[0.4em] mb-8 relative z-10 leading-none">Logistics Audit</p>
              <div className="space-y-6 relative z-10">
                <div>
                  <div className="flex items-center gap-3 text-white/40 mb-2">
                    <Clock size={14} /> <span className="text-[9px] font-medium uppercase tracking-[0.2em]">Initiated At</span>
                  </div>
                  <p className="text-xs font-medium text-white/80">{new Date(transport.created_at).toLocaleString()}</p>
                </div>
                <div className="pt-6 border-t border-white/10">
                  <div className="flex items-center gap-3 text-white/40 mb-2">
                    <History size={14} /> <span className="text-[9px] font-medium uppercase tracking-[0.2em]">Last Dispatch Sync</span>
                  </div>
                  <p className="text-xs font-medium text-white/80">{new Date(transport.updated_at).toLocaleString()}</p>
                </div>
              </div>
            </div>

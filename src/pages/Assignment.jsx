      ...
      <div className="min-h-screen bg-gray-100 px-8 py-14 lg:px-20" style={{ fontFamily: "'DM Mono', monospace" }}>
        ...
        <div className="flex flex-wrap items-end gap-6 mb-14">
          <div className="flex-1 min-w-[250px] space-y-3">
            <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-sm text-[9px] font-medium uppercase tracking-[0.18em] text-white bg-[#1a1916]">
              <Zap size={10} strokeWidth={3} /> Operations Command
            </div>
            <h1 className="text-5xl lg:text-6xl font-medium text-[#1a1916] leading-none" style={{ fontFamily: "'Sora', sans-serif", letterSpacing: '-0.03em' }}>
              Assignments
            </h1>
          ...
          {(isAdding || isEditing) && (
            ...
            >
              <div className="px-8 py-6 border-b-[1.5px] border-[#e2e0d8]">
                <h3 className="text-[14px] font-medium text-[#1a1916]" style={{ fontFamily: "'Sora', sans-serif" }}>
              ...
              <form onSubmit={handleSubmit} className="p-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                ...
                {currentUser?.role_name !== 'Staff' && (
                  <div className="lg:col-span-3 border-t border-[#f0efe9] pt-8 mt-2">
                    <h4 className="text-[9px] font-medium uppercase tracking-[0.2em] text-[#b0aea5] mb-6">Linked References (Optional)</h4>
                ...
                <div className="lg:col-span-3 flex justify-end gap-3 mt-4">
                  <button type="submit" disabled={isLoading} className="h-12 px-10 rounded-lg bg-[#1a1916] text-white text-[10px] font-medium uppercase tracking-[0.15em] transition-all hover:scale-[1.02] disabled:opacity-50">
          ...
          {!(isAdding || isEditing) && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="bg-white rounded-2xl border-[1.5px] border-[#e2e0d8] overflow-hidden">
              <div className="p-7 border-b-[1.5px] border-[#e2e0d8] flex items-center justify-between">
                ...
                <div className="text-[10px] font-medium text-[#78776f] uppercase tracking-widest bg-gray-100 px-4 py-2 rounded-lg border border-[#e2e0d8]">
                  ...
                  <thead>
                    <tr className="bg-[#f5f4f0]">
                      {['Personnel', 'Task Type', 'Assigned For', 'Status', 'Actions'].map((h, idx) => (
                        <th key={h} className={`px-8 py-5 text-[9px] font-medium uppercase tracking-[0.15em] text-[#78776f] border-b-[1.5px] border-[#e2e0d8] ${idx === 4 ? 'text-right' : ''}`}>
                          {h}
                        </th>
                      ))}
                    </tr>
                    ...
                    {paginatedData.length > 0 ? paginatedData.map((a) => {
                        ...
                        <tr key={a.id} className="hover:bg-[#fcfbf9] transition-colors group">
                          <td className="px-8 py-6">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-xl bg-[#1a1916] flex items-center justify-center text-white text-[11px] font-medium">
                                {staff.name.charAt(0)}
                              </div>
                              <div>
                                <div className="text-[12px] font-medium text-[#1a1916]" style={{ fontFamily: "'Sora', sans-serif" }}>{staff.name}</div>
                          ...
                          <td className="px-8 py-6">
                            <div className="inline-flex items-center gap-2 px-2.5 py-1 rounded bg-gray-100 border border-[#e2e0d8]">
                              ...
                              <span className="text-[9px] font-medium uppercase tracking-wider text-[#1a1916]">{task.name}</span>
                            </div>
                          </td>
                          <td className="px-8 py-6">
                          ...
                          <td className="px-8 py-6">
                            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-[9px] font-medium uppercase tracking-wider" style={{ background: status.bg, color: status.text, border: `1.5px solid ${status.border}` }}>
                      ...
                      <tr>
                        <td colSpan={5} className="py-20 text-center">
                          ...
                          <p className="text-[10px] font-medium uppercase tracking-[0.2em] text-[#c5c2b8]">Zero missions dispatched</p>
                        </td>
                      </tr>
                    )}
                  </tbody>
...
const FormField = ({ label, icon, children }) => (
  <div className="flex flex-col gap-1.5">
    <label className="text-[9px] font-medium uppercase tracking-[0.2em] text-[#78776f] ml-1">{label}</label>
...
const RefBadge = ({ icon, label }) => (
  <div className="flex items-center gap-1.5 px-2 py-1 rounded bg-[#ffffff] border border-[#e2e0d8] shadow-sm">
    <div className="text-[#b0aea5]">{icon}</div>
    <span className="text-[9px] font-medium text-[#78776f]">{label}</span>
  </div>
);

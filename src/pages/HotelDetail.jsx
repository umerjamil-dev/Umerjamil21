        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em]">Hospitality Hook</span>
              <div className="w-1.5 h-1.5 rounded-full bg-[var(--desert-gold)]"></div>
              <span className="text-[10px] font-medium text-slate-900 uppercase tracking-widest">RECORD: {hotel.id}</span>
            </div>
            <h1 className="text-2xl font-manrope font-medium text-slate-900 tracking-tight">{hotel.hotel_name}</h1>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-2xl p-10 shadow-sm border border-slate-100 relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-12 relative z-10">
              ...
              <div className="flex items-center gap-2 px-4 py-1.5 bg-slate-50 text-slate-600 rounded-full text-[10px] font-medium uppercase tracking-widest border border-slate-200">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    <div className="p-4 bg-emerald-50 rounded-xl border border-emerald-100">
                      <p className="text-[9px] font-medium text-emerald-600 uppercase tracking-widest mb-1">Check-In</p>
                      <p className="text-sm font-medium text-slate-900">{hotel.check_in}</p>
                    </div>
                    <Navigation size={14} className="text-slate-300" />
                    <div className="p-4 bg-slate-50 rounded-xl border border-slate-200">
                      <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1">Check-Out</p>
                      <p className="text-sm font-medium text-slate-900">{hotel.check_out}</p>
                    </div>
                  </div>
                </div>
                <div>
                  ...
                  <p className="text-lg font-manrope font-medium text-slate-900 uppercase tracking-widest">{hotel.rooms || 'Standard Room'}</p>
                </div>
              </div>
              ...
              <div className="space-y-8">
                <div>
                  ...
                  <div className="flex items-center gap-2 text-slate-400 mt-2">
                    <span className="text-[9px] font-medium uppercase tracking-[0.2em]">Transaction: {hotel.transaction_type_name || 'Registry Sync'}</span>
                  </div>
                </div>
                <div>
                  <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-3">Master Booking</p>
                  <Link to={`/bookings/${hotel.booking_id}`} className="text-sm font-medium text-black hover:text-[var(--desert-gold)] transition-colors underline decoration-slate-200 decoration-2 underline-offset-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm flex flex-col justify-between">
              <div>
                ...
                <p className="text-xs font-medium text-slate-900 mb-2 uppercase tracking-tight">{hotel.package_title || 'Custom Arrangement'}</p>
              </div>
              <div className="px-3 py-1 bg-slate-50 text-[9px] font-medium text-slate-500 rounded-lg border border-slate-200 uppercase tracking-tighter w-fit">
            ...
            <div className="bg-[#1e293b] rounded-2xl p-8 shadow-xl text-white">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-2xl font-manrope font-medium tracking-tighter">Stay Active</p>
                  <p className="text-[10px] font-medium text-white/40 uppercase tracking-widest">Registry Verified</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="space-y-8">
          <div className="bg-[#020617] rounded-2xl p-8 text-white shadow-xl relative overflow-hidden group">
            ...
            <p className="text-[10px] font-medium text-white/30 uppercase tracking-[0.4em] mb-8 relative z-10 leading-none">Registry Audit</p>
            <div className="space-y-6 relative z-10">
              <div>
                <div className="flex items-center gap-3 text-white/40 mb-2">
                  <Clock size={14} /> <span className="text-[9px] font-medium uppercase tracking-[0.2em]">Initial Hook</span>
                </div>
                <p className="text-xs font-medium text-white/80">{new Date(hotel.created_at).toLocaleString()}</p>
              </div>
              <div className="pt-6 border-t border-white/10">
                <div className="flex items-center gap-3 text-white/40 mb-2">
                  <History size={14} /> <span className="text-[9px] font-medium uppercase tracking-[0.2em]">Last Reconciliation</span>
                </div>
                <p className="text-xs font-medium text-white/80">{new Date(hotel.updated_at).toLocaleString()}</p>
              </div>
            </div>
          </div>

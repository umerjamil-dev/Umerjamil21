...
const LeadDetail = () => {
    ...
    if (isLoading) return <div className="p-20 text-center font-manrope font-medium text-gray-400 animate-pulse tracking-widest uppercase text-xs">Retrieving Intelligence Dossier...</div>;
    if (!lead) return <div className="p-20 text-center font-manrope font-medium text-red-400 tracking-widest uppercase text-xs">Intelligence Outdated: Lead not found.</div>;
                    ...
                    <div className="flex items-center gap-6">
                        ...
                        <div>
                            <div className="flex items-center gap-3 mb-1">
                                <span className="text-xs font-medium text-gray-400 uppercase tracking-widest">Lead Profile</span>
                                <div className="w-1 h-1 rounded-full bg-gray-300"></div>
                                <span className="text-xs font-medium text-gray-900">{lead.id}</span>
                            </div>
                            <h1 className="text-2xl font-manrope font-medium text-gray-900">{lead.lead_name}</h1>
                        </div>
                    </div>
                    <div className="flex items-center gap-4">
                        <button
                            onClick={handleDeleteLead}
                            className="px-5 py-2.5 bg-red-50 text-red-600 rounded-xl text-xs font-medium uppercase tracking-wider hover:bg-red-100 transition-colors flex items-center gap-2"
                        >
                            <Trash2 size={16} /> Delete
                        </button>
                        <button
                            onClick={handleUpdateDetails}
                            className="px-8 py-3 bg-[#111827] text-white rounded-xl text-xs font-medium uppercase tracking-widest shadow-lg hover:shadow-xl hover:-translate-y-0.5 transition-all flex items-center gap-2"
                        >
                        ...
                        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100 relative overflow-hidden">
                            ...
                            <div className="flex items-center justify-between mb-8 relative z-10">
                                ...
                                <div className="relative group">
                                    <select
                                        ...
                                        className={`appearance-none px-4 py-1.5 pr-8 rounded-xl text-[10px] font-medium uppercase tracking-widest border outline-none cursor-pointer hover:shadow-md transition-all ${getStatusColor(currentStatus)}`}
                                    >
                                        {masterData.lead.map(s => (
                                            <option key={s.id || s} value={s.id || s} className="text-gray-900 font-medium bg-white">
                                                {s.name || s}
                                            </option>
                                        ))}
                                    </select>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 relative z-10">
                                <div className="space-y-6">
                                    <div className="group">
                                        <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                                            <Phone size={14} /> Contact Number
                                        </p>
                                        <p className="text-lg font-medium text-gray-900 group-hover:text-blue-600 transition-colors cursor-pointer">{lead.phone}</p>
                                    </div>
                                    <div className="group">
                                        <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-2 flex items-center gap-2">
                                        ...
                                        <p
                                            ...
                                            className="text-lg font-medium text-gray-900 group-hover:text-blue-600 transition-colors cursor-pointer truncate"
                                        >
                                            {lead.email}
                                        </p>
                                    </div>
                                ...
                                <div className="space-y-6">
                                    <div>
                                        ...
                                        <div className="inline-flex py-1.5 px-4 bg-gray-50 border border-gray-100 rounded-xl text-sm font-medium text-gray-900 shadow-sm">
                                            {lead.source_name}
                                        </div>
                                    </div>
                                    <div>
                                        ...
                                        <div className="flex items-center gap-3 p-2 bg-gray-50 rounded-xl border border-gray-100 w-max pr-4">
                                            ...
                                            <p className="text-sm font-medium text-gray-900">{lead.assigned_to_name}</p>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-8 pt-8 border-t border-gray-100 flex flex-col sm:flex-row items-center gap-4 relative z-10">
                                <a
                                    href={`tel:${lead.phone}`}
                                    className="w-full sm:flex-1 py-4 bg-gray-50 hover:bg-[#111827] text-gray-700 hover:text-white rounded-xl text-[11px] font-medium uppercase tracking-widest transition-all shadow-sm hover:shadow-md hover:-translate-y-1 flex items-center justify-center gap-2 group border border-gray-200 hover:border-[#111827]"
                                >
                                    <Phone size={18} className="group-hover:scale-110 transition-transform" /> Call Lead
                                </a>
                                <button
                                    onClick={() => navigate(`/whatsapp/${id}`)}
                                    className="w-full sm:flex-1 py-4 bg-emerald-50 hover:bg-emerald-500 text-emerald-700 hover:text-white rounded-xl text-[11px] font-medium uppercase tracking-widest transition-all shadow-sm hover:shadow-md hover:-translate-y-1 flex items-center justify-center gap-2 group border border-emerald-100 hover:border-emerald-500"
                                >
                            ...
                            <div className="mt-8 pt-8 border-t border-gray-100 relative z-10">
                                <h3 className="text-[10px] font-medium text-gray-400 uppercase tracking-widest mb-4 flex items-center gap-2">
                                        ...
                                        {lead.documents.map((docUrl, index) => {
                                            ...
                                            return (
                                                ...
                                                >
                                                    ...
                                                    <div className="flex items-center gap-2 text-[10px] font-medium uppercase tracking-widest text-[#111827] opacity-0 group-hover:opacity-100 transition-opacity">
                    ...
                    <div className="space-y-8">
                        ...
                        <div className="bg-[#111827] rounded-xl p-8 text-white shadow-xl relative overflow-hidden group">
                            ...
                            <div className="flex items-center gap-3 mb-6 relative z-10 text-gray-400">
                                <Clock size={16} />
                                <h3 className="text-[10px] font-medium uppercase tracking-widest">Next Action</h3>
                            </div>

                            <div className="relative z-10 mb-8">
                                <p className="text-xs font-medium text-gray-400 uppercase tracking-widest mb-2">Scheduled Follow-up</p>
                                <h2 className="text-3xl font-manrope font-medium tracking-tighter text-white">{lead.follow_up_date || 'No Date'}</h2>
                            </div>

                            <button className="w-full py-4 bg-white text-[#111827] rounded-xl text-xs font-medium uppercase tracking-widest hover:bg-gray-100 transition-colors shadow-lg relative z-10">
                                Mark as Completed
                            </button>
                        </div>
                        ...
                        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-100">
                            ...
                            <div className="space-y-8 relative">
                                ...
                                {lead.notes?.map((note) => (
                                    <div key={note.id} className="relative pl-12 group">
                                        ...
                                        <div>
                                            <p className="text-sm font-medium text-gray-900 mb-1">{note.text}</p>
                                ...
                                <div className="relative pl-12 opacity-50">
                                    ...
                                    <div>
                                        <p className="text-sm font-medium text-gray-800 mb-1">Lead Created in System</p>
                                        <p className="text-[10px] font-medium text-gray-400 uppercase tracking-wider">{new Date(lead.created_at).toLocaleDateString()}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="mt-8 pt-6 border-t border-gray-100">
                                ...
                                <button
                                    ...
                                    className="w-full mt-4 py-3 bg-gray-900 text-white rounded-xl text-xs font-medium uppercase tracking-widest hover:bg-black transition-colors"
                                >
                                    Save Note
                                </button>
                            </div>

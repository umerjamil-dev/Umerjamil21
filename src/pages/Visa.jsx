        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          {[
            ...
            <div key={idx} className="bg-white p-6 rounded-xl border border-slate-200 shadow-sm flex flex-col gap-2 relative overflow-hidden group hover:shadow-md transition-all">
              ...
              <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest relative z-10">{stat.label}</p>
              <h4 className={`text-3xl font-manrope font-medium tracking-tighter relative z-10 ${stat.color}`}>{stat.value}</h4>
            </div>
          ))}
        </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white border-b border-slate-100">
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/4">Applicant Narrative</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/4">Visa Credential</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/4">Clearance Status</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em]">Log Notes</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] text-right">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  <tr><td colSpan="5" className="py-20 text-center text-sm font-medium uppercase tracking-widest text-slate-400 opacity-50">Synchronizing with MOFA Gateway...</td></tr>
                ) : paginatedData.length === 0 ? (
                  <tr><td colSpan="5" className="py-20 text-center text-sm font-medium uppercase tracking-widest text-slate-400 opacity-50">No applications found in active cycle.</td></tr>
                ) : paginatedData.map((item) => (
                  <tr key={item.id} className="group hover:bg-slate-50 transition-all cursor-pointer">
                    <td className="px-8 py-6 text-black">
                      <div>
                        <p className="text-base font-manrope font-medium text-slate-900 tracking-tight leading-none mb-1.5">
                          {item.customer_full_name || 'Applicant Registry'}
                        </p>
                        <p className="text-[10px] text-slate-400 font-medium uppercase tracking-widest">{item.package_title || 'Custom Segment'}</p>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-6">
                      <span className={`px-4 py-2 rounded-full text-[9px] font-medium uppercase tracking-[0.2em] inline-flex items-center gap-2 ${item.status === 'Approved' ? 'bg-[var(--sacred-emerald)]/10 text-[var(--sacred-emerald)] border border-[var(--sacred-emerald)]/20' :

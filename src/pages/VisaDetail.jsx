          ...
          <div className="flex items-center gap-6">
            ...
            <div>
              <div className="flex items-center gap-3 mb-1">
                <span className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em]">Visa Protocol</span>
                <div className="w-1.5 h-1.5 rounded-full bg-[var(--sacred-emerald)]"></div>
                <span className="text-[10px] font-medium text-slate-900 uppercase tracking-widest">REG: {visa.id}</span>
              </div>
              <h1 className="text-2xl font-manrope font-medium text-slate-900 tracking-tight">Visa Verification Node</h1>
            </div>
          </div>
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white rounded-2xl p-10 shadow-sm border border-slate-100 relative overflow-hidden group">
              ...
              <div className="flex items-center justify-between mb-12 relative z-10">
                ...
                <span className="px-4 py-1.5 bg-emerald-50 text-[var(--sacred-emerald)] rounded-full text-[10px] font-medium uppercase tracking-widest border border-emerald-100 uppercase italic">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
                <div className="space-y-8">
                  <div>
                    ...
                    <p className="text-xl font-mono font-medium text-slate-900 tracking-tighter bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 w-fit uppercase tracking-[0.1em]">
                      {visa.visa_number}
                    </p>
                  </div>
                  <div>
                    <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-3">Operational Registry</p>
                    <p className="text-xs font-medium text-slate-900 px-4 py-2 bg-slate-50 rounded-xl border border-slate-100 w-fit uppercase tracking-widest">
                      ID: {visa.id}
                    </p>
                  </div>
                </div>
                ...
                <div className="space-y-8">
                  <div>
                    ...
                    <p className="text-[9px] font-medium text-slate-400 mt-1 tracking-widest uppercase">Master Sync: PKR {parseFloat(visa.total_amount || 0).toLocaleString()}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-3">Booking Link</p>
                    <Link to={`/bookings/${visa.booking_id}`} className="text-sm font-medium text-black hover:text-[var(--desert-gold)] transition-colors underline decoration-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm flex flex-col justify-between">
                <div>
                  ...
                  <p className="text-xs font-medium text-slate-900 mb-2 uppercase tracking-tight">{visa.package_title || 'Custom Arrangement'}</p>
                </div>
              ...
              <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm flex flex-col justify-between">
                ...
                <p className="text-[11px] font-medium text-emerald-600 uppercase tracking-[0.2em] bg-emerald-50 px-4 py-3 rounded-xl border border-emerald-100 w-fit">
              ...
              <div className="space-y-6 relative z-10">
                <div>
                  <div className="flex items-center gap-3 text-white/40 mb-2">
                    <Clock size={14} /> <span className="text-[9px] font-medium uppercase tracking-[0.2em]">Initial Entry</span>
                  </div>
                  <p className="text-xs font-medium text-white/80">{new Date(visa.created_at).toLocaleString()}</p>
                </div>
                <div className="pt-6 border-t border-white/10">
                  <div className="flex items-center gap-3 text-white/40 mb-2">
                    <History size={14} /> <span className="text-[9px] font-medium uppercase tracking-[0.2em]">Last Verification</span>
                  </div>
                  <p className="text-xs font-medium text-white/80">{new Date(visa.updated_at).toLocaleString()}</p>
                </div>
              </div>
            </div>

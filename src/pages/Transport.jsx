            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white border-b border-slate-100">
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/4">Transit Node & Vector</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/5">Occupant Entity</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/4">Vehicle & Captain Index</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/5">Temporal Sync</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  <tr><td colSpan="5" className="py-20 text-center text-sm font-medium uppercase tracking-widest text-slate-400 opacity-50">Auditing Logistics Manifest...</td></tr>
                ) : paginatedData.length === 0 ? (
                  <tr><td colSpan="5" className="py-20 text-center text-sm font-medium uppercase tracking-widest text-slate-400 opacity-50">No transport records found for current cycle.</td></tr>
                  ...
                  <tr key={item.id} className="group hover:bg-slate-50 transition-all cursor-pointer">
                    <td className="px-8 py-6 text-black">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <p className="text-[12px] font-medium text-slate-900 uppercase tracking-widest leading-relaxed">{item.type}</p>
                          <p className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.2em] flex items-center gap-1.5 mt-1">
                    ...
                    <td className="px-8 py-6 text-black">
                      <div className="flex flex-col gap-2">
                        <span className="text-xs font-medium text-slate-700 bg-slate-100 border border-slate-200 px-3 py-1.5 rounded-xl inline-block w-fit">
                          {item.vehicle}
                        </span>
                        <span className="text-[10px] font-medium uppercase tracking-[0.1em] text-slate-500 flex items-center gap-2">
                    ...
                    <td className="px-8 py-6 text-black">
                      <div className="flex flex-col gap-1.5">
                        <span className="text-[10px] font-medium text-slate-600 uppercase tracking-widest flex items-center gap-2">
                          <Clock size={12} className="text-[var(--desert-gold)]" strokeWidth={3} />
                          {item.date}
                        </span>
                        <span className={`px-2 py-1 rounded inline-flex w-fit text-[9px] font-medium uppercase tracking-[0.2em] border ${item.status === 'Confirmed' ? 'bg-[var(--sacred-emerald)]/10 text-[var(--sacred-emerald)] border-[var(--sacred-emerald)]/20' :

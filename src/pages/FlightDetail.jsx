        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="text-[10px] font-black text-slate-400 uppercase tracking-[0.3em]">Flight Record</span>
              <div className="w-1.5 h-1.5 rounded-full bg-[var(--desert-gold)]"></div>
              <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest">ID: {flight.id}</span>
            </div>
            <h1 className="text-2xl font-manrope font-black text-slate-900 tracking-tight">Airline Node: {flight.airline}</h1>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="bg-white rounded-2xl p-10 shadow-sm border border-slate-100 relative overflow-hidden group">
            ...
            <div className="flex items-center justify-between mb-12 relative z-10">
              ...
              <span className="px-4 py-1.5 bg-emerald-50 text-[var(--sacred-emerald)] rounded-full text-[10px] font-black uppercase tracking-widest border border-emerald-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12 relative z-10">
              <div className="space-y-8">
                <div>
                  ...
                  <p className="text-xl font-mono font-bold text-slate-900 tracking-tighter bg-slate-50 px-4 py-2 rounded-xl border border-slate-100 w-fit">
                    {flight.ticket_number}
                  </p>
                </div>
                <div className="flex gap-12">
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Departure</p>
                    <p className="text-sm font-black text-slate-900">{flight.departure?.split('@')[1] || flight.departure || 'TBA'}</p>
                    <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">{flight.departure?.split('@')[0] || ''}</p>
                  </div>
                  <div>
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Arrival</p>
                    <p className="text-sm font-black text-slate-900">{flight.arrival?.split('@')[1] || flight.arrival || 'TBA'}</p>
                    <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-widest">{flight.arrival?.split('@')[0] || ''}</p>
                  </div>
                </div>
              </div>
              ...
              <div className="space-y-8">
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-[9px] font-black text-slate-400 mt-1 tracking-widest uppercase italic">Quota: {flight.pilgrim_quota} Units</p>
                  </div>
                </div>
                <div>
                  <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-3">Master Booking Reference</p>
                  <Link to={`/bookings/${flight.booking_id}`} className="text-sm font-black text-black hover:text-[var(--desert-gold)] transition-colors underline decoration-slate-200">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
             <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm flex flex-col justify-between">
                <div>
                   ...
                   <p className="text-xs font-black text-slate-900 mb-2 uppercase tracking-tight">{flight.package_title || 'Custom Arrangement'}</p>
                </div>
             ...
             <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm">
                ...
                <div className="space-y-4">
                   <div>
                      <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Registry Amount</p>
                      <p className="text-xl font-manrope font-black text-slate-900 tracking-tighter">PKR {parseFloat(flight.total_amount || 0).toLocaleString()}</p>
                   </div>
                   <div className="grid grid-cols-2 gap-4 pt-4 border-t border-slate-50">
                      <div>
                         <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Paid</p>
                         <p className="text-sm font-black text-[var(--sacred-emerald)]">{parseFloat(flight.paid_amount || 0).toLocaleString()}</p>
                      </div>
                      <div>
                         <p className="text-[8px] font-black text-slate-400 uppercase tracking-[0.2em] mb-1">Remaining</p>
                         <p className="text-sm font-black text-red-500">{parseFloat(flight.remaining_amount || 0).toLocaleString()}</p>
                      </div>
                   </div>
                </div>
             </div>
        ...
        <div className="space-y-8">
          <div className="bg-[#0f172a] rounded-2xl p-8 text-white shadow-xl relative overflow-hidden group">
            ...
            <p className="text-[10px] font-black text-white/30 uppercase tracking-[0.4em] mb-8 relative z-10 leading-none">System Audit</p>
            <div className="space-y-6 relative z-10">
              <div>
                <div className="flex items-center gap-3 text-white/40 mb-2">
                  <Clock size={14} /> <span className="text-[9px] font-black uppercase tracking-[0.2em]">Generated At</span>
                </div>
                <p className="text-xs font-bold text-white/80">{new Date(flight.created_at).toLocaleString()}</p>
              </div>
              <div className="pt-6 border-t border-white/10">
                <div className="flex items-center gap-3 text-white/40 mb-2">
                  <History size={14} /> <span className="text-[9px] font-black uppercase tracking-[0.2em]">Last Sync</span>
                </div>
                <p className="text-xs font-bold text-white/80">{new Date(flight.updated_at).toLocaleString()}</p>
              </div>
            </div>
          </div>

      ...
      <div className="space-y-12 animate-in fade-in duration-1000 font-inter pb-20">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-10 pb-8 border-b border-slate-200">
          ...
          <div className="flex items-center gap-4">
            <button className="flex items-center gap-2 px-6 py-5 bg-white border border-slate-200 rounded-xl text-[10px] font-medium text-slate-600 uppercase tracking-widest hover:border-black transition-all">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden relative">
          <div className="p-8 border-b border-slate-100 flex flex-wrap gap-4 items-center justify-between bg-slate-50/50">
            ...
            <div className="flex items-center gap-3">
              <button className="flex items-center gap-2 px-6 py-3 bg-white border border-slate-200 rounded-xl text-[10px] font-medium text-slate-600 uppercase tracking-widest hover:border-black transition-all">
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-white border-b border-slate-100">
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/5">Carrier Node</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/5">Passenger Quota</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/4">Departure/Arrival Vector</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] w-1/5">E-Ticket Ledger</th>
                  <th className="px-8 py-6 text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] text-right">Action</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-100">
                {isLoading ? (
                  <tr><td colSpan="5" className="py-20 text-center text-sm font-medium uppercase tracking-widest text-slate-400 opacity-50">Auditing Flight Manifest...</td></tr>
                ) : paginatedData.length === 0 ? (
                  <tr><td colSpan="5" className="py-20 text-center text-sm font-medium uppercase tracking-widest text-slate-400 opacity-50">No flight records found in current cycle.</td></tr>
                  ...
                  <tr key={item.id} className="group hover:bg-slate-50 transition-all cursor-pointer">
                    <td className="px-8 py-6 text-black">
                      <div className="flex items-center gap-4">
                        ...
                        <p className="text-base font-manrope font-medium text-slate-900 tracking-tight leading-none mb-1">{item.airline}</p>
                      </div>
                    </td>
                    <td className="px-8 py-6 text-black">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-2">
                        <div className="flex items-center gap-2 text-[10px] font-medium text-slate-600 uppercase tracking-widest px-2 py-1 rounded inline-flex w-fit bg-slate-50 border border-slate-200">
                          <Navigation size={10} className="transform rotate-45 text-[var(--desert-gold)]" />
                          A: {item.departure}
                        </div>
                        <div className="flex items-center gap-2 text-[10px] font-medium text-black uppercase tracking-widest px-2 py-1 rounded inline-flex w-fit bg-slate-50 border border-slate-200">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col gap-1.5 text-black">
                        <span className="text-xs font-mono font-medium text-slate-700 bg-slate-100 px-3 py-1.5 rounded-xl border border-slate-200 inline-block w-fit">
                          {item.ticket_number}
                        </span>
                        <span className={`text-[9px] font-medium uppercase tracking-[0.2em] inline-flex items-center gap-1.5 ${item.status === 'Ticketed' ? 'text-[var(--sacred-emerald)]' :

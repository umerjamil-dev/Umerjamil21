  ...
  <>
    <div className="p-20 flex flex-col items-center justify-center gap-6">
      ...
      <div className="text-center">
        <p className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.4em] mb-2">
          Protocol Failure
        </p>
      ...
      {onRetry && (
        <button
          onClick={onRetry}
          className="flex items-center gap-2 px-5 py-2.5 bg-slate-900 text-white text-[10px] font-medium uppercase tracking-widest rounded-xl hover:bg-slate-700 transition-all"
        >
...
const EmptyNode = ({ label }) => (
  <p className="text-[10px] font-medium text-slate-300 uppercase tracking-widest italic py-4 text-center border-2 border-dashed border-slate-50 rounded-xl">
    No {label} registered
  </p>
);
...
const BookingDetail = () => {
  ...
  if (!booking) {
    return (
      <div className="p-20 text-center font-manrope font-medium text-slate-400 tracking-widest uppercase text-xs">
        Protocol Not Found
      </div>
    );
  }
        ...
        <div className="flex items-center gap-6 relative z-10">
          ...
          <div>
            <div className="flex items-center gap-3 mb-1">
              <span className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.4em]">Master Protocol</span>
              <div className="w-1.5 h-1.5 rounded-full bg-[var(--desert-gold)]"></div>
              <span className="text-[10px] font-medium text-slate-900 uppercase tracking-widest">ID: {booking.booking_id || id}</span>
            </div>
            <h1 className="text-3xl font-manrope font-medium text-slate-900 tracking-tight">
              {customerName}
            </h1>
          </div>
        </div>
        <div className="flex items-center gap-4 relative z-10">
          <div className={`px-5 py-2 rounded-xl border text-[10px] font-medium uppercase tracking-widest ${booking.visa_status === 'Approved'
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        ...
        <div className="lg:col-span-8 space-y-10">
          ...
          <div className="bg-white rounded-2xl p-10 shadow-sm border border-slate-100 relative overflow-hidden group">
            ...
            <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-12">
              ...
              <div className="space-y-6">
                <h2 className="text-[10px] font-medium text-slate-400 uppercase tracking-[0.3em] flex items-center gap-3">
                  <User size={18} className="text-[var(--desert-gold)]" strokeWidth={2.5} /> Customer Intelligence
                </h2>
                <div>
                  <p className="text-2xl font-manrope font-medium text-slate-900 tracking-tight">{customerName}</p>
                  <p className="text-xs font-medium text-slate-400 mt-1 uppercase tracking-widest">
                    {booking.phone || booking.email || 'Client Primary Contact'}
                  </p>
                </div>
                <div className="pt-6 border-t border-slate-50">
                  <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-2">Package Allocation</p>
                  <p className="text-sm font-medium text-slate-900 flex items-center gap-2 italic">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <p className="text-4xl font-manrope font-medium text-slate-900 tracking-tighter leading-none">
                    ${safeAmount(booking.total_amount).toLocaleString()}
                  </p>
                  <p className="text-[9px] font-medium text-slate-400 mt-4 uppercase tracking-[0.2em]">Total Value Registry</p>
                </div>
                <div className="flex gap-10 pt-6 border-t border-slate-50">
                  <div>
                    <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1">Paid</p>
                    <p className="text-md font-medium text-[var(--sacred-emerald)]">
                      ${safeAmount(booking.paid_amount).toLocaleString()}
                    </p>
                  </div>
                  <div>
                    <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1">Remaining</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm hover:border-slate-300 transition-all group">
              <h3 className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-8 flex items-center gap-3">
                <Plane size={18} className="text-blue-500" strokeWidth={2.5} /> Flight Segment
              </h3>
              {booking.airline ? (
                <div className="space-y-4">
                  <div>
                    <p className="text-xs font-medium text-slate-900 mb-1">{booking.airline}</p>
                  ...
                  <div className="flex items-center gap-4 py-3 border-y border-slate-50">
                    <div className="text-center flex-1">
                      <p className="text-[8px] font-medium text-slate-400 uppercase tracking-widest mb-1">Departure</p>
                    ...
                    <div className="text-center flex-1">
                      <p className="text-[8px] font-medium text-slate-400 uppercase tracking-widest mb-1">Arrival</p>
                      <p className="text-[10px] font-medium text-slate-900">
            ...
            <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm hover:border-slate-300 transition-all group">
              ...
              {booking.visa_number ? (
                <div className="space-y-4">
                  <div>
                    <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest mb-1">E-Visa Protocol</p>
                    <p className="text-xs font-medium text-slate-900">{booking.visa_number}</p>
                  </div>
              ...
              {booking.hotel_name || booking.makkah_hotel || booking.madinah_hotel ? (
                <div className="space-y-4">
                  <div>
                    ...
                    <p className="text-[9px] font-medium text-slate-400 uppercase tracking-widest flex items-center gap-2">
                      <MapPin size={10} /> {booking.hotel_city || 'Saudi Arabia'}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-4 py-3 border-y border-slate-50">
                    <div>
                      <p className="text-[8px] font-medium text-slate-400 uppercase tracking-widest mb-1">Check-In</p>
                      <p className="text-[10px] font-medium text-slate-900">{booking.check_in || '—'}</p>
                    </div>
                    <div>
                      <p className="text-[8px] font-medium text-slate-400 uppercase tracking-widest mb-1">Check-Out</p>
                      <p className="text-[10px] font-medium text-slate-900">{booking.check_out || '—'}</p>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <p className="text-[9px] font-medium text-slate-900 uppercase tracking-wider bg-slate-50 px-3 py-1.5 rounded-lg w-fit">
              ...
              {booking.driver || booking.vehicle ? (
                <div className="space-y-4">
                  <div className="flex justify-between items-start">
                    ...
                    <div className="px-2 py-1 bg-slate-900 text-white text-[8px] font-medium rounded uppercase tracking-tighter">
                      {booking.plate_number || 'N/A'}
                    </div>
                  </div>
                  <div className="py-3 border-t border-slate-50 space-y-2">
                    <p className="text-[10px] font-medium text-slate-900 flex items-center gap-2">
                      <User size={12} className="text-slate-400" /> {booking.driver}
                    </p>
                    <p className="text-[9px] font-medium text-slate-400 flex items-center gap-2 ml-5 tracking-widest">
                      {booking.driver_phone || '—'}
                    </p>
                  </div>
                  <div className="flex items-center gap-3 pt-2">
                    <Clock size={12} className="text-slate-300" />
                    <span className="text-[9px] font-medium text-slate-900 uppercase tracking-widest">
        ...
        <div className="lg:col-span-4 space-y-10">
          ...
          <div className="bg-[#0f172a] rounded-2xl p-10 text-white shadow-xl relative overflow-hidden group">
            ...
            <p className="text-[10px] font-medium text-white/30 uppercase tracking-[0.4em] mb-10 relative z-10 font-manrope">System Synchronization</p>

            <div className="space-y-8 relative z-10">
              <div className="flex items-start gap-5">
                <div className="w-1.5 h-1.5 rounded-full bg-[var(--desert-gold)] mt-2 shadow-[0_0_8px_var(--desert-gold)]"></div>
                <div>
                  <p className="text-[9px] font-medium text-white/40 uppercase tracking-widest mb-2">Travel Window Activation</p>
                  <p className="text-xs font-medium text-white/90">{booking.travel_date || 'N/A'}</p>
                </div>
              </div>
              <div className="flex items-start gap-5 pt-8 border-t border-white/5">
                ...
                <div>
                  <p className="text-[9px] font-medium text-white/40 uppercase tracking-widest mb-2">Last Operational Check</p>
                  <p className="text-xs font-medium text-white/90">{new Date().toLocaleDateString()}</p>
                </div>
              </div>
              <div className="pt-10">
          ...
          <div className="bg-white rounded-2xl p-8 border border-slate-100 shadow-sm overflow-hidden relative">
            ...
            <h3 className="text-[10px] font-medium text-slate-900 uppercase tracking-widest mb-6 flex items-center gap-3">
              <Info size={16} className="text-[var(--desert-gold)]" strokeWidth={2.5} /> Node Intelligence
            </h3>
            <div className="space-y-6">
              <div>
                <p className="text-[8px] font-medium text-slate-400 uppercase tracking-widest mb-2 italic">Parent Package Index</p>
                <p className="text-xs font-medium text-slate-700">{booking.package_name || 'Standard Distribution'}</p>
              </div>
              <div className="pt-6 border-t border-slate-50">
                <p className="text-[8px] font-medium text-slate-400 uppercase tracking-widest mb-2 italic">Operational Status</p>
                <div className="flex items-center gap-3">
                  <div className="px-3 py-1 bg-slate-900 text-white text-[9px] font-medium rounded-lg uppercase tracking-widest">
                    Live
                  </div>

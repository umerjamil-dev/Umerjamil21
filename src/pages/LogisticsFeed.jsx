    ...
    <div className="min-h-screen bg-gray-100 px-6 py-12 lg:px-12" style={{ fontFamily: "'DM Mono', monospace" }}>
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div className="space-y-4">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-sm text-[10px] font-medium uppercase tracking-[0.2em] text-white bg-[#1a1916]">
            <Activity size={12} strokeWidth={3} /> Logistics Command Center
          </div>
          <h1 className="text-4xl lg:text-5xl font-medium text-[#1a1916] leading-none" style={{ fontFamily: "'Sora', sans-serif", letterSpacing: '-0.03em' }}>
            ...
            {paginatedData.length > 0 ? paginatedData.map((item) => (
              <tr key={item.assignment_id} className="hover:bg-[#fcfbf9] transition-colors group">
                ...
                <td className="px-6 py-5">
                  <div className="text-[12px] font-medium text-[#1a1916]">#{item.assignment_id}</div>
                </td>
                <td className="px-6 py-5">
                  <span className="text-[10px] font-medium uppercase tracking-wider text-[#78776f] bg-gray-100 px-2 py-1 rounded border border-[#e2e0d8]">
                    {item.task_type}
                  </span>
                </td>
                ...
                <td className="px-6 py-5">
                  <div className="space-y-1">
                    {item.visas?.map(v => (
                      <div key={v.id} className="text-[10px] font-medium text-[#1a1916] flex items-center gap-1">
                ...
                <td className="px-6 py-5 max-w-[200px]">
                  {item.packages?.map(p => (
                    <div key={p.id} className="space-y-0.5">
                      <div className="text-[11px] font-medium text-[#1a1916] truncate">{p.title}</div>
                      <div className="text-[9px] text-[#b0aea5] uppercase font-medium">{p.total_days} Days | ${p.final_price}</div>
                    </div>
                  ))}
                </td>
              </tr>
                ...
                <td colSpan={10} className="py-24 text-center">
                  {isLoading ? (
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <span className="text-[10px] font-medium uppercase tracking-widest text-[#b0aea5]">Retrieving Terminal Data...</span>
                    </div>
                  ) : (
                    <div className="flex flex-col items-center gap-2">
                      <Info size={32} className="text-[#e2e0d8]" />
                      <span className="text-[10px] font-medium uppercase tracking-widest text-[#b0aea5]">No synchronization found</span>
                    </div>
                  )}
                </td>
              </tr>
...
const TableHead = ({ label, icon }) => (
  <th className="px-6 py-5 text-[9px] font-medium uppercase tracking-[0.2em] text-[#78776f]">
...
const StatusBadge = ({ status }) => {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded text-[9px] font-medium uppercase tracking-wider" style={{ background: style.bg, color: style.text, border: `1.5px solid ${style.border}` }}>
...
const InfoCell = ({ title, subtitle, small }) => (
  <div className="space-y-1">
    <div className="text-[11px] font-medium text-[#1a1916] truncate" style={{ fontFamily: "'Sora', sans-serif" }}>{title || 'N/A'}</div>
    <div className="text-[10px] text-[#78776f] truncate leading-none">{subtitle}</div>
    {small && <div className="text-[9px] text-[#b0aea5] font-medium uppercase tracking-tighter truncate">{small}</div>}
  </div>
);
